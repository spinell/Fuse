...
TEST(Vec3, angleBetween) {
    ...
    // Test with custom unnormalized vector.
    {
        ...
        EXPECT_EQ(Vec3::kUnitX.angleBetween(vector), 45_deg);
        EXPECT_EQ(vector.angleBetween(Vec3::kUnitX), 45_deg);

        EXPECT_EQ(Vec3(1, 1, 0).angleBetween(vector), 90_deg);
        EXPECT_EQ(vector.angleBetween({1, 1, 0}), 90_deg);

        EXPECT_EQ(Vec3::kUnitY.angleBetween(vector), 135_deg);
        EXPECT_EQ(vector.angleBetween(Vec3::kUnitY), 135_deg);
        ...
        EXPECT_EQ(Vec3::kUnitXNeg.angleBetween(vector), 135_deg);
        EXPECT_EQ(vector.angleBetween(Vec3::kUnitXNeg), 135_deg);

        EXPECT_EQ(Vec3(-1, -1, 0).angleBetween(vector), 90_deg);
        EXPECT_EQ(vector.angleBetween({1, 1, 0}), 90_deg);

        EXPECT_EQ(Vec3::kUnitYNeg.angleBetween(vector), 45_deg);
        EXPECT_EQ(vector.angleBetween(Vec3::kUnitYNeg), 45_deg);
    }
#endif
}
